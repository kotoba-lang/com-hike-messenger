(ns hike-messenger.main
  "Kotodama WASM entrypoint for the Hike_messenger clean-room actor (L4) — Clojure port.
  ...
  facts (`hike_messenger.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hike_messenger")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"hike_messenger.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hike_messenger-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hike_messenger-compat/ready)
